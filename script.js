...
// Автоматическая подстройка размера карты под блок map-area
function resizeMap() {
...
function handleHexClick(hex) {
    ...
    const isNeighbor = getNeighbors(cH.q, cH.r).some(n => Number(n.q) === Number(hex.q) && Number(n.r) === Number(hex.r));
    if (!isNeighbor) {
